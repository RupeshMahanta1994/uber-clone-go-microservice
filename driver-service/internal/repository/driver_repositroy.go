...
func (r *DriverRepository) Create(ctx context.Context, driver *model.Driver) (string, error) {
	...
	_, err := r.db.Exec(ctx, query, driver.UserID, driver.VehicleNumber, driver.VehicleType, driver.Status, driver.IsAvailable, driver.Rating)
	if err != nil {
...
func (r *DriverRepository) GetByUserId(ctx context.Context, userId string) (*model.Driver, error) {
	query := `SELECT user_id,vehicle_number,vehicle_type,status,is_available,rating FROM drivers WHERE user_id=$1`
	row := r.db.QueryRow(ctx, query, userId)
	var driver model.Driver
	err := row.Scan(&driver.UserID, &driver.VehicleNumber, &driver.VehicleType, &driver.Status, &driver.IsAvailable, &driver.Rating)
	if err != nil {
...
func (r *DriverRepository) Update(ctx context.Context, driver *model.Driver) (string, error) {
	...
	_, err := r.db.Exec(ctx, query, driver.VehicleNumber, driver.VehicleType, driver.Status, driver.IsAvailable, driver.Rating)
	if err != nil {
